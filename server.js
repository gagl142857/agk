...
app.post("/GenieDungeon", async (req, res) => {
  try {
    ...
    const 지니 = 스탯생성(유저데이터.스탯.던전.지니.레벨);
    지니.스탯.치명 = 10 * 유저데이터.스탯.던전.지니.레벨;
    지니.스탯.치명피해 = 150 + 15 * 유저데이터.스탯.던전.지니.레벨;
...
app.post("/RockgolemDungeon", async (req, res) => {
  try {
    ...
    const 락골렘 = 스탯생성(유저데이터.스탯.던전.락골렘.레벨);
    락골렘.스탯.스턴 = 10 * 유저데이터.스탯.던전.락골렘.레벨;
    ...
    if (turn === "나") {
      ...
      if (Math.random() * 100 < 나.스탯.치명) {
        if (!Math.random() * 100 < 상대.스탯.치명무시) {
          최종데미지 = 기본데미지 * (나.스탯.치명피해 / 100) * ((Math.max(0, 100 - 나.스탯.치명저항)) / 100);
        } else {
          최종데미지 = 기본데미지 * (나.스탯.일반공격계수 / 100) * ((Math.max(0, 100 - 나.스탯.일반공격피해감소)) / 100);
        }
      } else {
        최종데미지 = 기본데미지 * (나.스탯.일반공격계수 / 100) * ((Math.max(0, 100 - 나.스탯.일반공격피해감소)) / 100);
