...
//   const 나회복량 = 나최대HP * ((나.스탯.치유량) * 0.01);
//   const 상대회복량 = 상대최대HP * ((상대.스탯.치유량) * 0.01);

//   let turn = 나.스탯.최종공속 >= 상대.스탯.최종공속 ? "나" : "상대";


//   let 나기본데미지 =
//     (나.스탯.최종공격력 - 상대.스탯.최종방어력)
//     * (Math.max(10, (100 - 상대.스탯.피해감소)) * 0.01)
//     * 나추뎀보정;

//   let 나관통데미지 =
//     (나.스탯.최종공격력
//       - (상대.스탯.최종방어력
//         * (Math.max(10, (100 - 나.스탯.관통)) * 0.01)))
//     * (Math.max(10, (100 - 상대.스탯.피해감소)) * 0.01)
//     * 나추뎀보정;

//   let 나일반 =
//     (나.스탯.일반공격계수 * 0.01)
//     * (Math.max(10, (100 - 상대.스탯.일반공격피해감소)) * 0.01);

//   let 나치명 =
//     (나.스탯.치명피해 * 0.01)
//     * (Math.max(10, (100 - 상대.스탯.치명저항)) * 0.01);

//   let 나콤보 =
//     (나.스탯.콤보계수 * 0.01)
//     * (Math.max(10, (100 - 상대.스탯.콤보피해감소)) * 0.01);

//   let 나반격 =
//     (나.스탯.반격계수 * 0.01)
//     * (Math.max(10, (100 - 상대.스탯.반격피해감소)) * 0.01);

//   let 나스킬치명 =
//     (나.스탯.스킬치명피해 * 0.01)
//     * (나.스탯.스킬피해 * 0.01)
//     * (Math.max(10, (100 - 상대.스탯.스킬피해감소)) * 0.01);
...
//     (나.스탯.동료치명피해 * 0.01)
//     * (나.스탯.동료피해 * 0.01)
//     * (Math.max(10, (100 - 상대.스탯.동료피해감소)) * 0.01);
...
//               나데미지효과 += " " + 나최종데미지;
//               나총데미지 += 나최종데미지;
//               전투로그.push({ 위치: 1, 내용: 나데미지효과 });
//             } else {
//               나데미지효과 += "[!치명]";
//               나데미지효과 += " " + 나최종데미지;
//               전투로그.push({ 위치: 1, 내용: 나데미지효과 });
//               상대발동효과 += "[회피]";
//               전투로그.push({ 위치: 0, 내용: 상대발동효과 });
//             }
//           } else {
//             상대.스탯.최종HP -= 나최종데미지;
//             나총데미지 += 나최종데미지;
...
//           상대.스탯.최종HP -= 나기본데미지 * 나스킬치명;
//           나총데미지 += 나기본데미지 * 나스킬치명;
//         } else {
//           상대.스탯.최종HP -= 나기본데미지 * 나스킬;
//           나총데미지 += 나기본데미지 * 나스킬;
//         }
...
//       if (나.스탯.동료1?.레벨 + 나.스탯.동료2?.레벨 + 나.스탯.동료3?.레벨) {
//         if (Math.random() * 100 < 나.스탯.동료치명) {
//           상대.스탯.최종HP -= 나기본데미지 * 나동료치명;
//           나총데미지 += 나기본데미지 * 나동료치명;
//         } else {
//           상대.스탯.최종HP -= 나기본데미지 * 나동료;
//           나총데미지 += 나기본데미지 * 나동료;
//         }
...
function 전투시뮬레이션(나, 상대) {
  ...
  while (나.스탯.최종HP > 0 && 상대.스탯.최종HP > 0) {
    ...
    if (turn === "나") {
      ...
      let 최종데미지 = 기본데미지 * 나추뎀보정 * Math.max(10, 100 - 상대.스탯.피해감소);
      ...
      let 최종데미지 = 기본데미지 * 상대추뎀보정 * Math.max(10, 100 - 나.스탯.피해감소);
