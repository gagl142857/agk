...
// 차단아이피관리
const 차단된IP목록 = ["",];
// "118.235.3.253", "118.235.14.1", "118.235.74.93", ""
...
app.post("/register", async (req, res) => {
  try {
    ...
    const 기본스탯 = {
      ...
      접속시각: Math.floor(new Date().getTime() / 3600000), // 478520,
...
app.post("/login", async (req, res) => {
  try {
    ...
    if (data.스탯.접속요일 !== 오늘요일) {
      ...
      if (data.스탯.월드보스.기회 < 2) data.스탯.월드보스.기회 = 2;
      data.스탯.던전.페어리.승패 = 1;
      data.스탯.전장.티켓 += 4;
      data.스탯.하늘섬 = 0;
      data.스탯.오늘하늘섬.루비 = 0;
      data.스탯.오늘하늘섬.크리스탈 = 0;
      data.스탯.오늘하늘섬.에메랄드 = 0;
      data.스탯.오늘하늘섬.사파이어 = 0;
      data.스탯.오늘하늘섬.자수정 = 0;
      data.스탯.오늘하늘섬.토파즈 = 0;
      data.스탯.오늘하늘섬.진주 = 0;
      // if (data.스탯.전장.티켓 == null) { // undefined 또는 null일 때만
      //   data.스탯.전장.티켓 = 4;
      // }
      ...
      if (내순위 === 1) {
        data.스탯.칭호 = { ...data.스탯.칭호, 오토봇의구원자: 0 };
      }
      ...
      const 미션목록 = [
        ...
        { 필요뎀: 666666, 보상: 2000 },
        { 필요뎀: 9999999, 보상: 3000 },
        { 필요뎀: 33333333, 보상: 4000 },
        { 필요뎀: 666666666, 보상: 5000 },
        { 필요뎀: 9999999999, 보상: 6000 },
        { 필요뎀: 33333333333, 보상: 7000 },
      ];

      let 달성단계 = 0;
      for (let i = 0; i < 미션목록.length; i++) {
        if (어제월드보스총뎀 >= 미션목록[i].필요뎀) 달성단계 = i + 1;
        else break;
      }
      const 보상다이아 = 달성단계 > 0 ? 미션목록[달성단계 - 1].보상 : 0;

      const 어제보스미션보상 = {
        이름: "다이아",
        수량: 보상다이아,
        시간: new Date().toLocaleString("ko-KR", { timeZone: "Asia/Seoul" }),
        메모: `월드보스 합동미션${달성단계}단계 일일보상`,
      };

      data.스탯.우편함.unshift(어제보스미션보상);

      await supabaseAdmin.from("로그기록").insert({
        스탯: data.스탯,
        유저아이디: data.스탯.계정.유저아이디,
        유저닉네임: data.스탯.계정.유저닉네임,
        내용: `월드보스 합동미션${달성단계}단계 일일보상(${보상다이아})`
      });
    ...
    if (data.스탯.계정.유저아이디 === "rkrmf") {
      data.스탯.다이아 += 10000;
    }

    if (data.스탯.계정.유저아이디 === "xptmxm") {
      data.스탯.다이아 += 10000;
    }
    ...
    if (!data.스탯.시간분) {
      data.스탯.시간분 = Math.floor(new Date().getTime() / 60000);
    }

    if (!data.스탯.시간초) {
      data.스탯.시간초 = Math.floor(new Date().getTime() / 1000);
    }
...
function 우편목록(data, mail) {
  if (mail.이름 === "램프") {
    ...
    return true;
  } else if (mail.이름 === "램프수거") {
    data.스탯.램프.수량 -= mail.수량;
    return true;
  } else if (mail.이름 === "열쇠꾸러미") {
    data.스탯.던전.지니.열쇠 += mail.수량;
    data.스탯.던전.로쿠규.열쇠 += mail.수량;
    data.스탯.던전.락골렘.열쇠 += mail.수량;
    data.스탯.던전.디지에그.열쇠 += mail.수량;
...
app.post("/ejswjsvnfwkehd", async (req, res) => {
  try {
    const { id } = req.body;
    if (!id) return res.status(400).json({ 오류: "id 필요" });
    ...
    if (!유저데이터.스탯.던전.지니.열쇠 && !유저데이터.스탯.던전.로쿠규.열쇠 && !유저데이터.스탯.던전.락골렘.열쇠 && !유저데이터.스탯.던전.디지에그.열쇠) {
      return res.status(400).json({ 오류: "열쇠가 부족합니다" });
    }


    while (유저데이터.스탯.던전.지니.열쇠 > 0) {
    ...
    while (유저데이터.스탯.던전.디지에그.열쇠 > 0) {
      const 디지에그 = 던전스탯생성(유저데이터.스탯.던전.디지에그.레벨);
      디지에그.스탯.계정.유저닉네임 = `디지에그`;
...
app.post("/Fairy", async (req, res) => {
  try {
    ...
    // //수정중
    // const 페어리 = 던전스탯생성(유저데이터.스탯.던전.페어리.레벨);
    // 페어리.스탯.최종HP = 1000 * 유저데이터.스탯.던전.페어리.레벨;
    // 페어리.스탯.최종공격력 = 60 * 유저데이터.스탯.던전.페어리.레벨;
    // 페어리.스탯.최종방어력 = 20 * 유저데이터.스탯.던전.페어리.레벨;
    // 페어리.스탯.최종공속 = 1 + 0.05 * 유저데이터.스탯.던전.페어리.레벨;
    // 페어리.스탯.전투력 =
    //   (1000 * 유저데이터.스탯.던전.페어리.레벨) * 0.05 +
    //   (60 * 유저데이터.스탯.던전.페어리.레벨) +
    //   (20 * 유저데이터.스탯.던전.페어리.레벨) * 2 +
    //   (1 + 0.05 * 유저데이터.스탯.던전.페어리.레벨) * 50;
    // 페어리.스탯.계정.유저닉네임 = `페어리`;


    const 페어리 = {
      스탯: {
        스킬: { 최고등급: Math.floor(유저데이터.스탯.던전.페어리.레벨 * 0.1) },
        동료1: { 레벨: 0 },
        동료2: { 레벨: 0 },
        동료3: { 레벨: 0 },
        계정: { 유저닉네임: `페어리` },
        ...
        일반공격계수: 100 + 1 * 1,
        일반공격피해감소: 1,
        스킬치명: Math.min(70, 1 * 유저데이터.스탯.던전.페어리.레벨),
        스킬치명피해: 150 + 1 * 유저데이터.스탯.던전.페어리.레벨,
        스킬피해: 유저데이터.스탯.던전.페어리.레벨,
        스킬피해감소: 1,
        ...
        막기무시: 1,
        피해감소: 1,
        추가데미지: Math.min(70, 1 * 1),
        최종HP: 1000 * 유저데이터.스탯.던전.페어리.레벨,
        최종공격력: 60 * 유저데이터.스탯.던전.페어리.레벨,
        최종방어력: 20 * 유저데이터.스탯.던전.페어리.레벨,
        최종공속: 1 + 0.05 * 유저데이터.스탯.던전.페어리.레벨,
        전투력:
          (1000 * 유저데이터.스탯.던전.페어리.레벨) * 0.05 +
          (60 * 유저데이터.스탯.던전.페어리.레벨) +
          (20 * 유저데이터.스탯.던전.페어리.레벨) * 2 +
...
app.post("/vozlwl", async (req, res) => {
  try {
    ...
    유저데이터.스탯.다이아 = 유저데이터.스탯.다이아 - 3300;
...
app.post("/vpdjflrlghl", async (req, res) => {
  try {
    const { id } = req.body;
...
app.post("/worldbossattack", async (req, res) => {
  try {
    ...
    const 메가트론 = 던전스탯생성(40);
    메가트론.스탯.계정.유저닉네임 = `메가트론`;
    메가트론.스탯.최종공속 = 0.5;
...
app.post("/dnjfqhehwjs", async (req, res) => {
  try {
    const { id } = req.body;
    if (!id) return res.status(400).json({ 오류: "id 필요" });
...
app.post("/wjswkdfltmxm", async (req, res) => {
  try {
    const { id } = req.body;
    if (!id) return res.status(400).json({ 오류: "id 필요" });
...
app.post("/wjswkdehwjs", async (req, res) => {
  try {
    const { id } = req.body;
    if (!id) return res.status(400).json({ 오류: "id 필요" });
    ...
    const 내전투력 = 유저데이터.스탯.전투력 || 0;

    // const 하위유저 = 전체유저
    //   .filter(u => (u.스탯.전투력 || 0) < 내전투력)
    //   .sort((a, b) => b.스탯.전투력 - a.스탯.전투력)
    //   .slice(0, 5);
    ...
    const 하위유저 = 전체유저
      .filter(u => {
        if (!u.스탯 || !u.스탯.전투력 || !u.스탯.접속시각 || u.스탯.주인장인가) return false
        const 경과시간 = Math.floor(new Date().getTime() / 3600000) - u.스탯.접속시각
        return (u.스탯.전투력 || 0) < 내전투력 && 경과시간 < 72
      })
      .sort((a, b) => b.스탯.전투력 - a.스탯.전투력)
      .slice(0, 5);
    ...
    if (하위유저.length === 0) {
      ...
      랜덤상대 = 하위유저[Math.floor(Math.random() * 하위유저.length)];
    }

    const 전투결과 = 전투시뮬레이션(
      JSON.parse(JSON.stringify(유저데이터)),
      JSON.parse(JSON.stringify(랜덤상대))
    );
...
function 전투시뮬레이션(나, 상대) {
  ...
  let 나스킬발동보정 = 0;
  if (나.스탯.직업종류 === "신술" && 나.스탯.직업.차수 > 3) {
    나스킬발동보정 = 5;
  }

  let 상대스킬발동보정 = 0;
  if (상대.스탯.직업종류 === "신술" && 상대.스탯.직업.차수 > 3) {
    상대스킬발동보정 = 5;
  }

  let 나동료발동보정 = 0;
  if (나.스탯.직업종류 === "궁술" && 나.스탯.직업.차수 > 3) {
    나동료발동보정 = 5;
  }

  let 상대동료발동보정 = 0;
  if (상대.스탯.직업종류 === "궁술" && 상대.스탯.직업.차수 > 3) {
    상대동료발동보정 = 5;
  }
  ...
  const 나회복량 = 나.스탯.최종HP * (나.스탯.치유량 * 0.01);
  const 상대회복량 = 상대.스탯.최종HP * (상대.스탯.치유량 * 0.01);

  let turn = 나.스탯.최종공속 >= 상대.스탯.최종공속 ? "나" : "상대";

  // let 나치명발동 = Math.random() * 100 < 나.스탯.치명;

  const 나치명 = 나.스탯.치명;
  const 나치명무시 = 나.스탯.치명무시;
  const 나치명피해 = 나.스탯.치명피해;
  const 나치명저항 = 나.스탯.치명저항;
  const 나콤보 = 나.스탯.콤보;
  const 나콤보무시 = 나.스탯.콤보무시;
  const 나콤보계수 = 나.스탯.콤보계수;
  const 나콤보피해감소 = 나.스탯.콤보피해감소;
  const 나반격 = 나.스탯.반격;
  const 나반격무시 = 나.스탯.반격무시;
  const 나반격계수 = 나.스탯.반격계수;
  const 나반격피해감소 = 나.스탯.반격피해감소;
  const 나스턴 = 나.스탯.스턴;
  const 나스턴무시 = 나.스탯.스턴무시;
  const 나회피 = 나.스탯.회피;
  const 나회피무시 = 나.스탯.회피무시;
  const 나회복 = 나.스탯.회복;
  const 나회복무시 = 나.스탯.회복무시;
  const 나스킬치명 = 나.스탯.스킬치명;
  const 나스킬치명피해 = 나.스탯.스킬치명피해;
  const 나스킬피해 = 나.스탯.스킬피해;
  const 나스킬피해감소 = 나.스탯.스킬피해감소;
  const 나동료치명 = 나.스탯.동료치명;
  const 나동료치명피해 = 나.스탯.동료치명피해;
  const 나동료피해 = 나.스탯.동료피해;
  const 나동료피해감소 = 나.스탯.동료피해감소;
  const 나일반공격계수 = 나.스탯.일반공격계수;
  const 나일반공격피해감소 = 나.스탯.일반공격피해감소;
  const 나관통 = 나.스탯.관통;
  const 나관통무시 = 나.스탯.관통무시;
  const 나피해감소 = 나.스탯.피해감소;
  const 나추가데미지 = 나.스탯.추가데미지;
  const 나최종공격력 = 나.스탯.최종공격력;
  const 나최종방어력 = 나.스탯.최종방어력;
  let 나현재HP = 나.스탯.최종HP;
  const 나스킬 = 나.스탯.스킬.최고등급 || 0;
  const 나동료 = (나.스탯.동료1.레벨 + 나.스탯.동료2.레벨 + 나.스탯.동료3.레벨) || 0;;


  const 상대치명 = 상대.스탯.치명;
  const 상대치명무시 = 상대.스탯.치명무시;
  const 상대치명피해 = 상대.스탯.치명피해;
  const 상대치명저항 = 상대.스탯.치명저항;
  const 상대콤보 = 상대.스탯.콤보;
  const 상대콤보무시 = 상대.스탯.콤보무시;
  const 상대콤보계수 = 상대.스탯.콤보계수;
  const 상대콤보피해감소 = 상대.스탯.콤보피해감소;
  const 상대반격 = 상대.스탯.반격;
  const 상대반격무시 = 상대.스탯.반격무시;
  const 상대반격계수 = 상대.스탯.반격계수;
  const 상대반격피해감소 = 상대.스탯.반격피해감소;
  const 상대스턴 = 상대.스탯.스턴;
  const 상대스턴무시 = 상대.스탯.스턴무시;
  const 상대회피 = 상대.스탯.회피;
  const 상대회피무시 = 상대.스탯.회피무시;
  const 상대회복 = 상대.스탯.회복;
  const 상대회복무시 = 상대.스탯.회복무시;
  const 상대스킬치명 = 상대.스탯.스킬치명;
  const 상대스킬치명피해 = 상대.스탯.스킬치명피해;
  const 상대스킬피해 = 상대.스탯.스킬피해;
  const 상대스킬피해감소 = 상대.스탯.스킬피해감소;
  const 상대동료치명 = 상대.스탯.동료치명;
  const 상대동료치명피해 = 상대.스탯.동료치명피해;
  const 상대동료피해 = 상대.스탯.동료피해;
  const 상대동료피해감소 = 상대.스탯.동료피해감소;
  const 상대일반공격계수 = 상대.스탯.일반공격계수;
  const 상대일반공격피해감소 = 상대.스탯.일반공격피해감소;
  const 상대관통 = 상대.스탯.관통;
  const 상대관통무시 = 상대.스탯.관통무시;
  const 상대피해감소 = 상대.스탯.피해감소;
  const 상대추가데미지 = 상대.스탯.추가데미지;
  const 상대최종공격력 = 상대.스탯.최종공격력;
  const 상대최종방어력 = 상대.스탯.최종방어력;
  let 상대현재HP = 상대.스탯.최종HP;
  const 상대스킬 = 상대.스탯.스킬.최고등급 || 0;;
  const 상대동료 = (상대.스탯.동료1.레벨 + 상대.스탯.동료2.레벨 + 상대.스탯.동료3.레벨) || 0;;


  const 나기본뎀 = Math.max(
    100,
    (나최종공격력 -
      (상대최종방어력 * 최소보정(Math.max(0, (나관통 - 상대관통무시)))))
    * ((100 + 나추가데미지) * 0.01)
    * 최소보정(상대피해감소)
  );
  ...
  const 상대기본뎀 = Math.max(
    100,
    (상대최종공격력 -
      (나최종방어력 * 최소보정(Math.max(0, (상대관통 - 나관통무시)))))
    * ((100 + 상대추가데미지) * 0.01)
    * 최소보정(나피해감소)
  );
  ...
  //   내체력: 유저HP,
  //   내최대체력: 유저.최대체력,
  //   상대닉네임: 상대.스탯.계정.유저닉네임,
  //   상대체력: 보스전 ? 몬스터.체력 : 몬스터HP,
  //   상대최대체력: 몬스터.체력,
  //   전투로그.turn: ``,
  //   상대나치명로그: ``,
  // });
  ...
  //       div.style.alignSelf = 행동.주체 === "나" ? "flex-start" : "flex-end";
  //       div.innerText = 행동.내용;
  //       전투로그컨테이너.appendChild(div);
  //     });
  //   });
  // });
  ...
  while (나현재HP > 0 && 상대현재HP > 0) {
    ...
    if (turn === "나") {
      ...
      if (발동(상대회복) && !발동(나회복무시)) {
        상대현재HP += 상대회복량;
        if (상대현재HP > 상대최대HP) 상대현재HP = 상대최대HP;
        상대총회복량 += 상대회복량;
      }
...
//     //내턴
//     if (turn === "나") {
//       let 기본데미지 = Math.max(100, 나.스탯.최종공격력 - 상대.스탯.최종방어력);
//       if (!Math.random() * 100 < 상대.스탯.관통무시) {
//         기본데미지 = Math.max(100, 나.스탯.최종공격력 - 상대.스탯.최종방어력 * ((Math.max(0, 100 - 나.스탯.관통)) / 100));
//       }
//       let 최종데미지 = 기본데미지 * 나추뎀보정 * Math.max(0.1, 100 - 상대.스탯.피해감소);
...
//           let 반격데미지 = Math.max(100, 상대.스탯.최종공격력 - 나.스탯.최종방어력);
//           반격데미지 = 반격데미지
//             * (상대.스탯.일반공격계수 / 100)
//             * (상대.스탯.반격계수 / 100)
...
//       //상대턴
//       let 기본데미지 = Math.max(100, 상대.스탯.최종공격력 - 나.스탯.최종방어력);
//       if (!Math.random() * 100 < 나.스탯.관통무시) {
//         기본데미지 = Math.max(100, 상대.스탯.최종공격력 - 나.스탯.최종방어력 * ((Math.max(0, 100 - 상대.스탯.관통)) / 100));
//       }
//       let 최종데미지 = 기본데미지 * 상대추뎀보정 * Math.max(0.1, 100 - 나.스탯.피해감소);
...
//           let 반격데미지 = Math.max(100, 나.스탯.최종공격력 - 상대.스탯.최종방어력);
//           반격데미지 = 반격데미지
//             * (나.스탯.일반공격계수 / 100)
//             * (나.스탯.반격계수 / 100)
//             * ((Math.max(0, 100 - 상대.스탯.반격피해감소)) / 100);
...
//       //스킬발동
//       if (Math.random() * 100 < (20 + 상대스킬발동보정)) {
...
//       //동료발동
//       if (Math.random() * 100 < (20 + 상대동료발동보정)) {
